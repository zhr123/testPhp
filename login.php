...
<p><input type="checkbox" name="savePwd" />
记住密码
</p>
...
if($submit!=""){
	...
	$sql="SELECT * FROM user WHERE userID = '$userID' and permission='user';";
	...
	if(mysql_num_rows($result)==1){
		...
		if(mysql_num_rows($result)==1){
			...
			setcookie("permission","user",time()+3600);
			if($savePwd) setcookie("permission","user",time()+3600*24*7);
			
			//跳转页面
			echo "<script language='javascript'>";
			echo "document.location='loginSuccess.html'";
			echo "</script>";
		}
		else {
		...
		echo "<span class='error'>没有此用户</span>";
	}
